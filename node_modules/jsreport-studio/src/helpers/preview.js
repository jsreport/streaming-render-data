...
export default async function (request, target) {
  ...
  // NOTE: NEW implementation based on fetch
    ...
    if (response.status === 200) {
      ...
      await parseStreamingMultipart(response, (fileInfo) => {
        if (fileInfo.name !== 'report') {
          ...
          // TODO: verify if using blob vs filename makes some difference in the preview of
          // files like pdf, probably when using file there is more metada because of the filename
          // but who knows..

          // const file = new Blob([fileInfo.rawData], { type: fileInfo.contentType })
          const file = new File([fileInfo.rawData], fileInfo.filename || '', {
            type: fileInfo.contentType
          })

          const localFileSrc = window.URL.createObjectURL(file)

          target.changeSrc(localFileSrc)
        }
      })
    } else {
      let content
  ...
  // NOTE: OLD implementation based on FORMS
  /*
