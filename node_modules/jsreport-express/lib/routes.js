...
module.exports = (app, reporter, exposedOptions) => {
  ...
  app.post('/api/report/:name?', (req, res, next) => {
    ...
    if (req.query.logsStreaming === 'true') {
      console.log('LOGS STREAMING ENABLED...')
      ...
      form.append('log1', formatLog('testing log'), { contentType: 'application/json' })
      form.append('log2', formatLog('another log'), { contentType: 'application/json' })
      form.append('log3', formatLog('and another log'), { contentType: 'application/json' })

      setTimeout(() => {
        form.append('log4', formatLog('x2testing log'), { contentType: 'application/json' })
        form.append('log5', formatLog('x2another log'), { contentType: 'application/json' })
        form.append('log6', formatLog('x2and another log'), { contentType: 'application/json' })
      }, 1000)

      setTimeout(() => {
        form.append('log7', formatLog('x3testing log'), { contentType: 'application/json' })
        form.append('log8', formatLog('x3another log'), { contentType: 'application/json' })
        form.append('log9', formatLog('x3and another log'), { contentType: 'application/json' })
      }, 2000)

      setTimeout(() => {
        const min = 1
        const max = 1000
        const reportContent = Buffer.from(`<h1>some output content, id: ${Math.floor(Math.random() * (max - min)) + min}</h1>`)
        const outputStream = Readable.from(reportContent)

        form.append('report', outputStream, {
